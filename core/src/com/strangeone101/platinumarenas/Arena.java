...
public class Arena {
    ...
    public void reset(int resetSpeed, CommandSender sender, boolean silent) {
        ...
        data.sectionAmounts = new int[getSections().size()];
        data.sectionIDs = new int[getSections().size()];
        data.sectionCount = getSections().size();
        for (Section s : getSections()) {
            ...
            data.sectionAmounts[s.getID()] = sectionAmount; //Store the amount of blocks each section should reset per tick
            data.sectionIDs[s.getID()] = s.getID();
        }
    ...
    private void loopyReset(ResetLoopinData data, CommandSender sender) {
        ...
        int sectionsIterated = 0;
        int activeIndex = data.currentSectionResetting;
        while (sectionsIterated < data.sectionCount && data.sectionCount > 0) {
            if (activeIndex >= data.sectionCount) activeIndex = 0;

            int id = data.sectionIDs[activeIndex];
            Section s = getSections().get(id);
            long t = System.nanoTime();
            boolean reset = s.reset(data.sectionAmounts[id]);
            data.resetMicroseconds += System.nanoTime() - t;
            if (reset) {
                t = System.nanoTime();
                data.removeSectionAt(activeIndex);
                ...
                data.recalculateSectionAmounts(getSections());
                data.calculateMicroseconds += System.nanoTime() - t;
            } else {
                activeIndex++;
                sectionsIterated++;
            ...
            if (data.blocksThisTick > data.maxBlocksThisTick) {
                if (data.sectionCount > 0) {
                    data.currentSectionResetting = activeIndex % data.sectionCount;
                }
                data.blocksThisTick += s.getBlocksResetThisTick();
                break;
            }
    ...
    public static Arena createNewArena(String name, Location corner1, Location corner2, Player player) {
        ...
        data.keyList = new ArrayList<>(Arrays.asList(arena.keys));
        data.keyIndexes = new HashMap<>();
        for (int i = 0; i < data.keyList.size(); i++) {
            data.keyIndexes.put(data.keyList.get(i), (short)i);
        }
    ...
    public void setResetSpeed(int speed) {
        if (isBeingReset()) {
            ...
            this.currentReset.recalculateSectionAmounts(getSections());
        }
    }
    ...
    private static class CreationLoopinData {
        ...
        Arena arena;
        ShortArrayBuilder blockAmounts = new ShortArrayBuilder();
        ShortArrayBuilder blockTypes = new ShortArrayBuilder();
        Map<Integer, Pair<Wrapper, Object>> NBT = new HashMap<>();
        List<BlockData> keyList;
        Map<BlockData, Short> keyIndexes;

        short getOrAddKey(BlockData blockData, Player player) {
            Short existing = keyIndexes.get(blockData);
            ...
            short index = (short)keyList.size();
            keyList.add(blockData);
            keyIndexes.put(blockData, index);
            return index;
        }
    }

    private static class ShortArrayBuilder {
        private short[] values = new short[64];
        private int size;
    ...
    private static class ResetLoopinData {
        ...
        void removeSectionAt(int index) {
            int amountToMove = sectionCount - index - 1;
            if (amountToMove > 0) {
                System.arraycopy(sectionIDs, index + 1, sectionIDs, index, amountToMove);
            }
            sectionCount--;
            if (currentSectionResetting >= sectionCount && sectionCount > 0) {
                currentSectionResetting = 0;
            }
        }

        void recalculateSectionAmounts(List<Section> allSections) {
            int totalRemainingBlocks = 0;
            for (int i = 0; i < sectionCount; i++) {
                totalRemainingBlocks += allSections.get(sectionIDs[i]).getTotalBlocks();
            }

            if (totalRemainingBlocks == 0) return;

            for (int i = 0; i < sectionCount; i++) {
                Section section = allSections.get(sectionIDs[i]);
                int sectionAmount = (int) ((double)speed / (double)totalRemainingBlocks * (double)section.getTotalBlocks());
                if (sectionAmount <= 0) sectionAmount = 1; //Do AT LEAST one block per tick in each section
                sectionAmounts[section.getID()] = sectionAmount;
            }
        }
    }
    ...
    private static void loopyCreate(CreationLoopinData data, final int amount, Player player, Runnable onFinished) {
        ...
        for (int i = 0; i < amount; i++) {
            if (data.index >= width * height * length) {
                ...
                data.sections.add(new Section(data.arena, data.sections.size(), start, end, data.blockTypes.toArray(), data.blockAmounts.toArray(), data.NBT));
                data.blockAmounts.clear();
                data.blockTypes.clear();
                data.sectionStarts.remove(0);
                data.sectionEnds.remove(0);
                data.NBT = new HashMap<>();
                data.arena.setKeys(data.keyList);
            ...
            int x = data.index % width;
            int y = data.index / (length * width);
            int z = (data.index / width) % length;
            Block block = world.getBlockAt(startX + x, startY + y, startZ + z);
            BlockData blockData = block.getBlockData();
            short blockKeyIndex = data.getOrAddKey(blockData, player);
            ...
            Wrapper wrapper = WrapperRegistry.getFromMaterial(block.getType());
            //Test if the block has custom NBT
            PlatinumArenas.debug("Wrapper is " + wrapper + " for " + block.getType() + " index " + data.index);
            ...
            if (wrapper != null) {
                ...
                if (state instanceof TileState) {
                    ...
                    blank = wrapper.isBlank(tileState);
                }
            }

            if (data.blockTypes.size() == 0) {
                data.blockAmounts.add((short)1);
                data.blockTypes.add(blockKeyIndex);
                //PlatinumArenas.INSTANCE.getLogger().info("Debug4: Creating array thingies");

                //Record the NBT if it has any
                if (tileState != null && !blank) {
                    Object cache = wrapper.cache(tileState); //Cache the NBT from the tilestate
                    PlatinumArenas.debug("Cache is " + cache + " for " + block.getType());
            ...
            if (data.blockTypes.getLast() == blockKeyIndex) { //If the last block recorded is the same type
                data.blockAmounts.incrementLast();
                ...
                if (data.blockAmounts.getLast() == Short.MAX_VALUE) { //We have reached the max amount for this array
                    data.blockTypes.add(blockKeyIndex); //Add the same type to the array so we can start the count again from 0
                    data.blockAmounts.add((short)0); //Start the count from 0 again
                }

                continue;
            }

            PlatinumArenas.debug("Blank is " + blank + " for " + block.getType());
            if (tileState != null && !blank) {
                Object cache = wrapper.cache(tileState); //Cache the NBT from the tilestate
                PlatinumArenas.debug("Cache is " + cache + " for " + block.getType());
                data.NBT.put(data.index, new ImmutablePair<>(wrapper, cache)); //Store in the section that the current block has the current NBT
