...
public class Section {
    ...
    public boolean reset(int max) {
        ...
        int startX = getStart().getBlockX();
        int startY = getStart().getBlockY();
        int startZ = getStart().getBlockZ();
        ...
        while (resetTypeIndex < blockTypes.length)
        {
            ...
            {
                int x = resetLocationIndex % w;
                int y = resetLocationIndex / (l * w);
                int z = (resetLocationIndex / w) % l;
                ...
                if (NBT_CACHE.containsKey(resetLocationIndex)) { //If the NBT cache has this location
                    if (!(block.getState() instanceof TileState)) { //If the placed block doesn't have NBT somehow???
                        ...
                        if (!wrapper.getTileClass().isAssignableFrom(block.getState().getClass())) { //If the blocktype doesnt match
                            ...
                            try {
                                wrapper.place((TileState) block.getState(), cache).update(true); //The update part is important or else the tilestate won't change
                            } catch (Exception e) {
    ...
    protected byte[] getNBTData() {
        ...
        for (Wrapper wrapper : wrapperTypes.keySet()) {
            ...
            for (Object cache : wrapperTypes.get(wrapper).keySet()) {
                ...
                //again.
                for (int indexStart = 0; indexStart < indexes.size(); indexStart += 127) {
                    int amountOfIndexes = Math.min(127, indexes.size() - indexStart);

                    out.writeByte((byte) amountOfIndexes); //How many indexes use this object (in this iteration, at least)

                    for (int i = 0; i < amountOfIndexes; i++) {
                        out.writeInt(indexes.get(indexStart + i)); //Write an index
                    }
